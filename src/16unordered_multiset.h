//使用hash_table来实现，hash_table已经实现了unordered_multiset的方法
...
namespace qyk
{
    ...
    public:
        ...
        using self = unordered_multiset<T, HashFunc, KeyEqual, Alloc>;
        ...
        unordered_multiset() : core(50, HashFunc(), KeyEqual()){}
        explicit unordered_multiset(const size_type n) : core(n, HashFunc(), KeyEqual()) {}
        unordered_multiset(const size_type n, const HashFunc& hf) : core(n, hf, KeyEqual()){}
        unordered_multiset(const size_type n, const HashFunc& hf, const KeyEqual& eql) : core(n, hf, eql){}
        template<class Iterator, typename = typename my_enable_if<is_iterator<Iterator>::value>::type>
		unordered_multiset(Iterator first, Iterator last) : core(50, HashFunc(), KeyEqual()) {
			core.insert_equal(first, last);
		}
		template<class Iterator, typename = typename my_enable_if<is_iterator<Iterator>::value>::type>
		unordered_multiset(Iterator first, Iterator last, size_type n)
