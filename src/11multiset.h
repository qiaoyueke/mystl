...
namespace qyk
{
    ...
    public:
        ...
        using self = multiset<T, Compare, Alloc>;
        ...
        multiset() : core(Compare()){}
        explicit multiset(Compare comp) : core(comp) {}
        template <class Iterator, typename = typename my_enable_if<is_iterator<Iterator>::value>::type>
        multiset(Iterator first, Iterator last) : core(Compare())
        {
            core.insert_equal(first, last);
        }
        multiset(const self &other) : core(other.core) {}
