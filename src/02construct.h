...
namespace qyk {
	...
	namespace detail {
		...
		inline void __destroy_aux(ForwardIterator first, ForwardIterator last, __false_type) {
			for (; first != last; ++first) {
				qykDestroy(&*first); //传入的是一个指针，指向first这个迭代器指向的元素，这个函数执行：(&*first)->~T();
			}
		}
	...
	template<typename ForwardIterator>
	inline void qykDestroy(ForwardIterator first, ForwardIterator last) {
	...
	inline void qykDestroy(char*, char*) { }
	inline void qykDestroy(wchar_t*, wchar_t*) { }
